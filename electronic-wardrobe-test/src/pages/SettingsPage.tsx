...
export default function SettingsPage({
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-lg font-bold text-gray-900">我的</h2>
      ...
      <section className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            ...
            <p className="text-xs text-gray-500">总衣物数</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-2xl font-bold text-gray-900">
              {outfits.length}
            </p>
            <p className="text-xs text-gray-500">搭配方案数</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-2xl font-bold text-gray-900">
              {availableItems.filter((i) => i.isFavorite).length}
            </p>
            <p className="text-xs text-gray-500">收藏衣物</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-2xl font-bold text-gray-900">
              {outfits.filter((o) => o.isFavorite).length}
            </p>
            <p className="text-xs text-gray-500">收藏搭配</p>
          </div>
        </div>

        {/* 品类分布 */}
        <div className="mb-4">
          <h4 className="text-xs font-medium text-gray-500 mb-2">品类分布</h4>
        ...
        {topColors.length > 0 && (
          <div>
            <h4 className="text-xs font-medium text-gray-500 mb-2">常用颜色</h4>
        ...
        <section className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            {topWorn.filter((i) => i.wearCount > 0).length === 0 && (
              <p className="text-xs text-gray-400">暂无穿着记录</p>
            )}
          </div>
        </section>
      )}
      ...
      {/* 闲置提醒 */}
      {idleItems.length > 0 && (
        <section className="bg-orange-50 rounded-xl border border-orange-100 p-4">
          ...
          <div className="flex flex-wrap gap-1">
            {idleItems.map((item) => (
              <span
                key={item.id}
                className="text-xs px-2 py-1 bg-white text-orange-700 rounded-full border border-orange-200"
              >
                {item.subCategory}
              </span>
            ))}
